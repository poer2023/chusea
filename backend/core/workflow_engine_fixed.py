...
class WorkflowEngine:
    ...
    def setup_tasks(self):
        ...
        def plan_document(self, document_id: str, user_prompt: str):
            """Plan阶段: 生成文档大纲"""
            return asyncio.run(self._plan_document_async(document_id, user_prompt))
            
        @self.celery.task(bind=True)
        def draft_document(self, document_id: str, outline: str):
            """Draft阶段: 基于大纲生成内容"""
            return asyncio.run(self._draft_document_async(document_id, outline))
            
        @self.celery.task(bind=True)
        def check_citations(self, document_id: str, content: str):
            """Citation阶段: 验证和格式化引用"""
            return asyncio.run(self._check_citations_async(document_id, content))
            
        @self.celery.task(bind=True)
        def check_grammar(self, document_id: str, content: str):
            """Grammar阶段: 语法检查和修正"""
            return asyncio.run(self._check_grammar_async(document_id, content))
            
        @self.celery.task(bind=True)
        def check_readability(self, document_id: str, content: str):
            """Readability阶段: 可读性检测"""
            return asyncio.run(self._check_readability_async(document_id, content))
    
    async def _plan_document_async(self, document_id: str, user_prompt: str):
        """Plan阶段异步实现"""
        try:
            ...
            async with self.async_session() as db:
                ...
                try:
                    ...
                    # 触发下一阶段
                    self.celery.send_task('workflow.draft_document', args=[document_id, outline])
    ...
    async def _draft_document_async(self, document_id: str, outline: str):
        """Draft阶段异步实现"""
        try:
            ...
            async with self.async_session() as db:
                ...
                try:
                    ...
                    # 触发引用检查
                    self.celery.send_task('workflow.check_citations', args=[document_id, content])
    ...
    async def _check_citations_async(self, document_id: str, content: str):
        """Citation阶段异步实现"""
        try:
            ...
            async with self.async_session() as db:
                ...
                try:
                    ...
                    # 触发语法检查
                    self.celery.send_task('workflow.check_grammar', args=[document_id, formatted_content])
    ...
    async def _check_grammar_async(self, document_id: str, content: str):
        """Grammar阶段异步实现"""
        try:
            ...
            async with self.async_session() as db:
                ...
                try:
                    ...
                    # 触发可读性检查
                    self.celery.send_task('workflow.check_readability', args=[document_id, corrected_content])
    ...
    async def _check_readability_async(self, document_id: str, content: str):
        """Readability阶段异步实现"""
        try:
