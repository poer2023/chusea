...
export const WorkflowExample: React.FC = () => {
  const { currentWorkflow, isActive, currentStep, overallProgress } = useWorkflow();
  const { startWorkflow, pauseWorkflow, resumeWorkflow, cancelWorkflow } = useWorkflowActions();
  const { nextStep, previousStep, completeCurrentStep, canGoToNextStep, canGoToPreviousStep } = useWorkflowSteps();
  const { isAIProcessing, generateWithAI } = useWorkflowAI();
