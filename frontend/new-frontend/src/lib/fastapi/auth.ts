...
import { FASTAPI_ENDPOINTS, FASTAPI_ERROR_CODES } from './endpoints';
...
export class FastAPIAuthManager {
   ...
   * 用户注册
   */
  public async register(userData: FastAPIRegisterRequest): Promise<void> {
    this.updateAuthState({ isLoading: true });
    ...
    try {
      ...
      const registeredUser: FastAPIUser = await response.json();
