  ...
  const handleWorkflowAction = async (action: string, data?: any) => {
    ...
    try {
      ...
      addMessage({
        ...
        role: 'system',
        content: `Workflow ${action}: ${workflow.type}`,
        timestamp: Date.now(),
        status: 'delivered',
        metadata: {
          type: 'workflow',
    ...
    <Card className={cn('p-4 space-y-4', className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="font-medium text-sm">{workflow.type}</h3>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span className="font-medium">{Math.round(workflow.progress * 100)}%</span>
        </div>
