...
export function useChunkedUpload() {
  ...
  const uploadChunk = async (
    ...
    return apiClient.upload('/files/upload-chunk', formData, {
      additionalData: {},
      validate: false,
    });
  };
...
export function useBatchFileOperations() {
  ...
  return {
    ...
    batchCompress,
    batchDownload,
    isLoading: batchUpload.isLoading || 
               batchDelete.isLoading || 
               batchMove.isLoading || 
               batchCompress.isLoading || 
               batchDownload.isLoading,
  };
}
...
export function useFileProcessing() {
  ...
  return {
    ...
    isProcessing: processFile.isLoading || 
                  convertFile.isLoading || 
                  extractText.isLoading || 
                  generateThumbnail.isLoading || 
                  analyzeFile.isLoading,
  };
}
...
export function useFileSharing() {
  ...
  return {
    ...
    isLoading: shareFile.isLoading || 
               createPublicLink.isLoading || 
               revokeShare.isLoading,
  };
}
