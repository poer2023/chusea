...
export function useAdvancedPermissions() {
  ...
  const checkPermission = React.useCallback((
    ...
    const contextualPermissions = user.contextualPermissions || {};
  ...
  const checkRole = React.useCallback((
    ...
    if (context && user.contextualRoles) {
      const orgRoles = user.contextualRoles[context.organizationId || ''];
      const teamRoles = user.contextualRoles[context.teamId || ''];
  ...
  const checkQuota = React.useCallback((
    ...
    const quota = user?.subscription?.quota?.[quotaType];
  ...
  const getEffectivePermissions = React.useCallback((): string[] => {
    ...
    if (user.contextualRoles) {
      Object.values(user.contextualRoles).flat().forEach(role => {
...
export function useBiometricAuth() {
  ...
  return {
    ...
    isLoading: registerBiometric.isLoading || 
               authenticateWithBiometric.isLoading || 
               removeBiometric.isLoading,
  };
}
...
// Add storage key for offline actions
...
if (typeof APP_CONFIG !== 'undefined') {
  (APP_CONFIG.STORAGE_KEYS as any).OFFLINE_ACTIONS = 'chusea_offline_actions';
}