...
export function useWorkflowProgress(workflowId: string) {
  ...
  const progress = React.useMemo((): WorkflowProgress | null => {
    ...
    const completedSteps = steps.filter(step => step.status === NodeStatus.PASS).length;
    const percentage = totalSteps > 0 ? (completedSteps / totalSteps) * 100 : 0;
    
    const currentStep = steps.find(step => step.status === NodeStatus.RUNNING);
    const nextStep = steps.find(step => step.status === NodeStatus.PENDING);
    ...
    return {
      ...
      timeline: workflow.metadata?.timeline || null,
