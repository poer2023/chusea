...
import { useWorkflowStore, WorkflowState } from '@/stores/workflow-store';
import { SlashCommand, ChatSession, TypingIndicator as TypingIndicatorType } from '@/types/chat';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const { createSession, sendMessage, connect, executeCommand, updateMessage, deleteMessage, retryMessage } = chatStore;
  ...
  const handleSystemAction = useCallback((action: string, data?: unknown) => {
    switch (action) {
      case 'view-workflow':
  ...
  const activeTypingUsers = typingUsers.filter((t: TypingIndicatorType) => t.isTyping);
  ...
  return (
    <Card className={cn('flex flex-col', maxHeight, className)}>
      ...
      <ConnectionStatus 
        ...
        latency={(wsProps as { metrics?: { latency?: number } })?.metrics?.latency || 0}
      />
