...
import { useChatStore, useChatActions, ChatStoreState } from '@/stores/chat-store';
...
export const useChatWebSocket = (options: UseChatWebSocketOptions = {}) => {
  ...
  const chatActions = useChatActions() as any;
  const { addMessage, updateTypingUsers, setConnectionStatus, setError } = chatActions;
  ...
  const sendChatMessage = useCallback(async (content: string, role: 'user' | 'assistant' = 'user') => {
    ...
    const success = sendWebSocketMessage(websocketMessage as any);
  ...
  const sendTypingIndicator = useCallback((isTyping: boolean) => {
    ...
    sendWebSocketMessage(typingMessage as any);
  ...
  const sendWorkflowCommand = useCallback((command: string, params: any = {}) => {
    ...
    return sendWebSocketMessage(commandMessage as any);
  }, [
    enableWorkflowUpdates,
    user,
    currentSession,
  ...
  const joinSession = useCallback((targetSessionId: string) => {
    ...
    return sendWebSocketMessage(joinMessage as any);
  ...
  const leaveSession = useCallback((targetSessionId: string) => {
    ...
    return sendWebSocketMessage(leaveMessage as any);
  }, [user, sendWebSocketMessage]);
