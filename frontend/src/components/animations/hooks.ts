...
export function useAnimationConfig(defaultConfig: Record<string, string | number | boolean>) {
...
export function useResponsiveAnimation() {
  ...
  const getAnimationConfig = (baseConfig: Record<string, string | number | boolean>) => {
    switch (screenSize) {
      case 'mobile':
        return {
          ...baseConfig,
          duration: Math.max((baseConfig.duration as number) * 0.7, 200),
          delay: Math.max((baseConfig.delay as number) * 0.5, 0),
        };
      case 'tablet':
        return {
          ...baseConfig,
          duration: Math.max((baseConfig.duration as number) * 0.85, 250),
          delay: Math.max((baseConfig.delay as number) * 0.75, 0),
        };
      default:
        return baseConfig;
    }
